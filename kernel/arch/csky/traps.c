...
#define MFCR_DISTI 0xC0006020
	...
	if((tinstr & MTCR_MASK) == MTCR_DISTI)
	{
		...
		if(index_regx <= 13)
		{
			regx = *((int*)regs + (index_regx + 3));
		}else if(index_regx < 32){
			regx = *((int*)regs + (index_regx + 2));
		}else{
			goto bad_or_fault;
		}
	...
	else if((tinstr & MFCR_MASK) == MFCR_DISTI)
	{
		...
		if(index_regx <= 13){
			addr = (unsigned long*)regs + (index_regx + 3);
			*addr = regx;
		} else if(index_regx < 32){
			addr = (unsigned long*)regs + (index_regx + 2);
			*addr = regx;
		} else {
			goto bad_or_fault;
		}
...
bad_or_fault:
	return 0;
#else /* not define CONFIG_CPU_HAS_FPU */
	return 0;
