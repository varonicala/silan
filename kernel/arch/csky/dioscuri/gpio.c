/* linux/arch/csky/dioscuri/gpio.c
 *
...
static int ck_gpiolib_to_irq(struct gpio_chip* chip, unsigned offset)
{
	if(chip->base == 0x00 && offset <= DIOSCURI_GPIOA10_IRQ )
	{
		return offset;
	}
	else
...
void __init ck_gpio_init(void)
{
	...
	for(i = 0; i < GPIO_BANK_NUM; i++)
	{
		...
		ck_gpio->regbase = CK_GPIO_BASE + 0x0c*i;
...
void __init ck_gpio_irq_init(void)
{
	...
	for(irq = DIOSCURI_GPIOA0_IRQ; irq <= DIOSCURI_GPIOA10_IRQ; irq++)
	{
