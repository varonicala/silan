...
/* Select the default MAC&PHY */
#ifdef CONFIG_SILAN_IVS1
#define STMMAC_RESOURCE_NAME	"stmmac1eth"
#define PHY_RESOURCE_NAME		"stmmac1phy"
#define MAC_IRQ_NAME			"mac1irq"
#define MAC_CLK_NAME			"gmac1"
#else
#define STMMAC_RESOURCE_NAME	"stmmaceth"
#define PHY_RESOURCE_NAME		"stmmacphy"
#define MAC_IRQ_NAME			"macirq"
#define MAC_CLK_NAME			"gmac"
#endif

/* Weight of NAPI
 * The speed of our GMAC is 100Mbps, a value smaller than the original 64
 * will be better */
...
static int stmmac_open(struct net_device *dev)
{
	...
	if (unlikely(ret < 0)) {
		pr_err("%s: ERROR: allocating the IRQ %d (name:%s, error: %d)\n",
		       __func__, dev->irq, dev->name, ret);
		goto open_error;
	}
...
static int stmmac_probe(struct net_device *dev)
{
	...
	priv->clk = clk_get(&dev->dev, MAC_CLK_NAME);
	if (IS_ERR(priv->clk)) 
	{
	...
	priv->pause = pause;
	netif_napi_add(dev, &priv->napi, stmmac_poll, GMAC_NAPI_WEIGHT);
...
static int stmmac_dvr_probe(struct platform_device *pdev)
{
	...
	ndev->irq = platform_get_irq_byname(pdev, MAC_IRQ_NAME);
	if (ndev->irq == -ENXIO) {
