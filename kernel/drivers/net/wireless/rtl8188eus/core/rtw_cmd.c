...
_func_enter_;
	...
	else
	{
		...
		plist = &obj->list;
		//printk("%d::plist: %p, plist->prev:%p, plist->next: %p, plist->next->prev: %p,  queue_num:%d\n", __LINE__, plist, plist->prev, plist->next, plist->next->prev, queue_num);
		queue_num--;
		
		if(plist->next <= 0x80000000 || plist->next->prev <= 0x8000000){
			tmp = plist;
			while(tmp->prev != plist){
				printk("%d::plist: %p, plist->prev:%p, plist->next: %p,  queue_num: %d\n", __LINE__, tmp, tmp->prev, tmp->next, queue_num);
				//printk("%d::plist: %p, plist->prev:%p, plist->next: %p, plist->next->prev: %p\n", __LINE__, tmp, tmp->prev, tmp->next, tmp->next->prev);
				tmp = tmp->prev;
			}
			plist->next = tmp;
			printk("%d::plist: %p, plist->prev:%p, plist->next: %p, plist->next->prev: %p\n", __LINE__, plist, plist->prev, plist->next, plist->next->prev);

			printk("[%d] head %p, tail %p, tail->prev->next %p[tail], tail->next %p[head]\n", __LINE__,
...
post_process:
	...
	do{
		...
		printk("%s: leaving... drop cmdcode:%u size:%d\n", __FUNCTION__, pcmd->cmdcode, pcmd->cmdsz);
