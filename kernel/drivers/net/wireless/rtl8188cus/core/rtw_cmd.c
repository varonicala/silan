...
_func_enter_;
	...
	pwriteptmparm = (struct writePTM_parm*)rtw_zmalloc(sizeof(struct writePTM_parm)); 
...
_func_enter_;
	...
	pwriteptmparm = (struct writePTM_parm*)rtw_zmalloc(sizeof(struct setdig_parm)); 
...
_func_enter_;
	...
	pwriteptmparm = (struct writePTM_parm*)rtw_zmalloc(sizeof(struct setra_parm)); 
...
_func_enter_;
...
#ifdef SET_STAKEY_WITHOUT_CMD
	rtl8192c_set_stakey_hdl(padapter,(u8*)psetstakey_para);
	rtw_setstaKey_cmdrsp_callback(padapter,  ph2c);
	...
	// set timer to restore the sw_encrypt setting
	_set_timer(&padapter->securitypriv.enable_hw_enc_timer, ENABLE_HW_ENC_TIMEOUT);
	#endif
...
_func_enter_;	
	...
	psetrfintfsparm = (struct setrfintfs_parm*)rtw_zmalloc(sizeof(struct setrfintfs_parm)); 
...
_func_enter_;
	...
	prdtssiparm = (struct readTSSI_parm*)rtw_zmalloc(sizeof(struct readTSSI_parm)); 
	if(prdtssiparm ==NULL){
		rtw_mfree((unsigned char *) ph2c, sizeof(struct	cmd_obj));
		return _FAIL;
	}
...
_func_enter_;	
	...
	psetassocsta_rsp = (struct set_assocsta_rsp*)rtw_zmalloc(sizeof(struct set_assocsta_rsp)); 
