...
static void pl011_dma_tx_callback(void *data)
{
	...
	writel(uap->dmacr, uap->port.membase + UART011_DMACR);
...
static inline bool pl011_dma_tx_start(struct uart_amba_port *uap)
{
	...
	if (readl(uap->port.membase + UART01x_FR) & UART01x_FR_TXFF) {
		/*
	...
	uap->dmacr = dmacr;
	writel(dmacr, uap->port.membase + UART011_DMACR);
...
skip_rx:
	...
	if (uap->vendor->dma_threshold)
		writel(ST_UART011_DMAWM_RX_16 | ST_UART011_DMAWM_TX_16,
...
#ifdef CONFIG_PM
static int silan_uart_suspend(struct platform_device *dev, pm_message_t mesg)
{
	struct uart_amba_port **uap = platform_get_drvdata(dev);
	int i = 0, ret = 0;
	...
	//for (i = 0; i < ARRAY_SIZE(amba_ports); i++)
		ret = uart_suspend_port(&silan_reg, &uap[i]->port);

	//for (i = 0; i < ARRAY_SIZE(amba_ports); i++)
		clk_disable(uap[i]->clk);

	return ret;
}

static int silan_uart_resume(struct platform_device *dev)
{
	struct uart_amba_port **uap = platform_get_drvdata(dev);
	int i = 0, ret = 0;
	...
	//for (i = 0; i < ARRAY_SIZE(amba_ports); i++)
		ret = uart_resume_port(&silan_reg, &uap[i]->port);

	//for (i = 0; i < ARRAY_SIZE(amba_ports); i++)
		clk_enable(uap[i]->clk);

	return ret;
}
#endif

#ifdef CONFIG_SILAN_UART_PORT2
...
#ifdef CONFIG_PM
	.suspend	= silan_uart_suspend,
	.resume		= silan_uart_resume,
#endif
};
#endif

#ifdef CONFIG_SILAN_UART_PORT3
static struct platform_driver silan_uart3_driver = {
	.driver = {
		.name	= "silan-uart3",
		.owner	= THIS_MODULE,
	},
...
#ifdef CONFIG_SILAN_UART_PORT4
static struct platform_driver silan_uart4_driver = {
	.driver = {
		.name	= "silan-uart4",
		.owner	= THIS_MODULE,
	},
...
{
	int ret;
	printk(KERN_INFO "Serial: PL011 UART driver on SILAN SUV\n");

	ret = uart_register_driver(&silan_reg);
	if (ret == 0) {
#ifdef CONFIG_SILAN_UART_PORT2
		ret = platform_driver_register(&pl011_driver);
		if (ret)
            goto end;
#endif        

#ifdef CONFIG_SILAN_UART_PORT3
		ret = platform_driver_register(&silan_uart3_driver);
		if (ret)
			goto end;
#endif
#ifdef CONFIG_SILAN_UART_PORT4
		ret = platform_driver_register(&silan_uart4_driver);
		if (ret)
			goto end;
#endif
...
#endif
#ifdef CONFIG_SILAN_UART_PORT3
	platform_driver_unregister(&silan_uart3_driver);
#endif
#ifdef CONFIG_SILAN_UART_PORT4
	platform_driver_unregister(&silan_uart4_driver);
#endif
    uart_unregister_driver(&silan_reg);
}
