...
static const char *s3c_ts_name = "Goodix-TS";
...
COORDINATE_POLL:
	...
	if(finger)
	{
		for(count=0; count<finger; count++)
		{
			...
			input_x = input_x*735/550;
			input_y = input_y*600/735;
			//input_y = input_y*550/735;
	
			if((input_x > ts->abs_y_max)||(input_y > ts->abs_x_max))continue;
