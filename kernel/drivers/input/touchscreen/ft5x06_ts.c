...
static int ft5x0x_read_data(void)
{
	...
	ret = ft5x0x_i2c_rxdata(src, CFG_POINT_READ_BUF);
    if (ret < 0) {
...
static void ft5x0x_report_value(void)
{
	...
	input_report_key(data->input_dev, BTN_TOUCH,   i > 0);
	input_sync(data->input_dev);
...
ft5x0x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err = request_irq(TOUCH_IRQ, ft5x0x_ts_interrupt, IRQF_TRIGGER_FALLING, "ft5x0x_ts", ft5x0x_ts);
	if (err < 0) {
