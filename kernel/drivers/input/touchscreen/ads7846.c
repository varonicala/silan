...
#define TS_POLL_PERIOD	80	/* ms delay between samples */
...
static int get_ads_x(int ads_x, int ads_y)
{
	int x;
	static int last_x = -1;
	if(ads_x == 0 && ads_y == 0)
	...
	{
		x = calibration[0] * ads_x + calibration[1]*ads_y+calibration[2];
		x /= calibration[6];
		if(x < 0)
			x = 0;
		else if(x > 800)
...
static int get_ads_y(int ads_x, int ads_y)
{
	int y;
	static int last_y = -1;
	if(ads_x == 0 && ads_y == 0)
	...
	{
		y = calibration[3] * ads_x + calibration[4]*ads_y+calibration[5];
		y /= calibration[6];
		if(y < 0)
			y = 0;
		else if(y > 480)
...
static int __devinit ads7846_probe(struct spi_device *spi)
{
	...
	snprintf(ts->name, sizeof(ts->name), "ADS%dTouchscreen", ts->model);
	...
	input_set_abs_params(input_dev, ABS_X,
			pdata->x_min ? : 100,//0,
			pdata->x_max ? : 3850,//MAX_12BIT,
			0, 0);
	input_set_abs_params(input_dev, ABS_Y,
			pdata->y_min ? : 300,//0,
			pdata->y_max ? : 3800,//MAX_12BIT,
			0, 0);
#endif
