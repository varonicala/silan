...
static int viu_start_generating(VIU_handle_t *handle)
{
	...
	VIU_printk("************viu_start_generating\n");
	VIU_OpenCLK();//you need to open clk before config registers
	//config registers
	VIU_ConfigCapMode(0,handle);//bt656
	//VIU_ConfigCapMode(2,handle);//480p embbded
	VIU_StartCapture(0);
	//VIU_StartCapture(2);//480p embbded
					...
					struct v4l2_format *f)
{
	...
	//TODO
#if 1
	handle->crop_info.width =handle->actual_width;
	handle->crop_info.height =handle->actual_height;
#else

	handle->crop_info.width =720;
	handle->crop_info.height =480;
...
{     
	...
	for(i = 0; i < VIU_DEF_BUFS; i++) {
		handle->lpvbits[i].base = dev->lpvbits[i].base;
		handle->lpvbits[i].phy_addr = dev->lpvbits[i].phy_addr;
		handle->lpvbits[i].size = dev->lpvbits[i].size;

	}

	handle->width = MAX_WIDTH;
	handle->height = MAX_HEIGHT;
...
void fillbuffer_tasklet(unsigned long arg)
{
	...
	VIU_printk("memcpy frame_index:%d vbuf:%p\n",frame_index,vbuf);
	//memcpy(vbuf, (void *)dev->lpvbits[frame_index].base,handle->frameSize);
	if(handle->storeFormat==Mode_Planar)
	{

		//planar case
		src =(unsigned char *)( dev->lpvbits[frame_index].base);
		dst = vbuf;
#if 0
		...
		src +=size;
#else
		for(i=0;i<handle->height_store;i++)
		{

			memcpy(dst,src,handle->y_width_store);
			src += handle->y_stride;
			dst += handle->y_width_store;
...
static int viu_probe(struct platform_device *pdev) 
{
	...
	*vfd = viu_template;
	...
	for(i = 0; i < VIU_DEF_BUFS; i++) {
		dev->lpvbits[i].base = 0;
		dev->lpvbits[i].phy_addr = 0;
		dev->lpvbits[i].size = MAX_FRAME_SIZE;
		ret = viu_alloc_dma_buffer(&(dev->lpvbits[i]));
		printk("dev->lpvbits[%d].phy_addr:%p\n",i,(void *)dev->lpvbits[i].phy_addr);
		if(ret) {
...
err:
	for(i = 0; i < VIU_DEF_BUFS; i++) {
		...
		if(dev->lpvbits[i].base)
		{
			viu_free_dma_buffer(&(dev->lpvbits[i]));
			dev->lpvbits[i].base = 0;
		}
