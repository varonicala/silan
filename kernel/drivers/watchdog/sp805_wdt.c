...
#define MODULE_NAME		"silan-watchdog"
...
static void wdt_setload(unsigned int timeout)
{
	...
	wdt->timeout = ((load+1)*2+rate/2)/rate;
...
sp805_wdt_probe(struct platform_device *adev)
{
	int ret = 0,size;
	struct resource *res;
	...
	res = platform_get_resource(adev,IORESOURCE_MEM,0);
	if(res==NULL)
	{
		printk(KERN_ERR"cannot get res IO resource\n");
		ret= -ENOENT;
	}
	
	size = (res->end - res->start)+1;
	
	wdt->wdt_mem = request_mem_region(res->start,size,adev->name);
	if(wdt->wdt_mem ==NULL)
	{
		ret = -ENOENT;
		goto err_req1;
	...
	wdt->base = ioremap(res->start,size);
	if(wdt->base ==NULL)
	{
		ret = -ENOMEM;
...
err_req1:
	if(wdt->wdt_mem)
	{
		release_resource(wdt->wdt_mem);
		kfree(wdt->wdt_mem);
	}
err_req2:
	if(wdt->base)  iounmap(wdt->base);
	return ret;
}

  static int __devexit sp805_wdt_remove(struct platform_device *adev)
{
	release_resource(wdt->wdt_mem);
	kfree(wdt->wdt_mem);
	wdt->wdt_mem = NULL;
...
static struct platform_driver silan_wdt_driver = {
...
{
	return platform_driver_register(&silan_wdt_driver);
}
module_init(sp805_wdt_init);

static void __exit sp805_wdt_exit(void)
{
	platform_driver_unregister(&silan_wdt_driver);
}
...
MODULE_AUTHOR("CHDSW@silan.com.cn");
MODULE_DESCRIPTION("Silan SP805 Watchdog Driver");
MODULE_LICENSE("GPL");
